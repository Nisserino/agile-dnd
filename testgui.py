...
class GuiTools():
    ...
    def registerationWindow(self):
        ...
        self.windowReg.geometry('800x600')
    ...
    def mainMenu(self):
        ...
        self.windowMenu.geometry('800x600')
        # radio buttons / buttons for choices
        # -----------BUTTONS
        self.choice1_btn = tk.Button(self.windowMenu, text='Create a new character', command=lambda: 'Menu_loop.do_new_character()')
        self.choice1_btn.grid(row=6, column=1, columnspan=2, padx=10, pady=10, ipadx=70)
        self.choice2_btn = tk.Button(self.windowMenu, text="Load an existing character", command=lambda: 'Menu_loop.do_load_character()')
        self.choice2_btn.grid(row=7, column=1, columnspan=2, padx=10, pady=10, ipadx=55)
        self.choice3_btn = tk.Button(self.windowMenu, text="Start the game", command=lambda: 'Menu_loop.do_start_game()')
